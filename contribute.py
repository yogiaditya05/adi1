...
def main(def_args=sys.argv[1:]):
    ...
    if repository is not None:
        url_path = repository[:-4] if repository.endswith('.git') else repository
        start = url_path.rfind('/') + 1
        directory = url_path[start:]
...
def arguments(argsval):
    ...
    parser.add_argument('-r', '--repository', type=str, required=False,
                        help='The URL of the remote Git repository to push to')
    parser.add_argument('-un', '--user_name', type=str, required=False,
                        help='The user name to configure locally')
    parser.add_argument('-ue', '--user_email', type=str, required=False,
                        help='The user email to configure locally')
    parser.add_argument('-mc', '--max_commits', type=int, default=10, required=False,
                        help='The maximum number of commits to make per day')
    parser.add_argument('-f', '--frequency', type=int, default=80, required=False,
                        help='The percentage of days on which commits will be generated')
    parser.add_argument('-nw', '--no_weekends', action='store_true', required=False,
                        help='Exclude weekends from commit generation')
    parser.add_argument('-db', '--days_before', type=int, default=365, required=False,
                        help='Number of days before current date to start commit generation')
    parser.add_argument('-da', '--days_after', type=int, default=0, required=False,
                        help='Number of days after current date to keep committing')
